...
resourcename="stockmarket"  # lower case, only
resourcegroup="stockmarket_rg"
...
# git clone https://github.com/apilogicserver/stockmarket.git
# cd stockmarket
...
az group create --name stockmarket_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group stockmarket_rg --sku S1 --is-linux
az container create --resource-group stockmarket_rg --name stockmarket --image apilogicserver/stockmarket:latest --dns-name-label stockmarket --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//stockmarket.westus.azurecontainer.io:5656'
